...
export default function MovieTable({ medias = [] }) {
  const { t } = useLanguage();
      ...
      <div className="table-responsive">
        <table className="table table-dark table-hover mb-0 align-middle">
          ...
          <tbody className="border-top-0">
            {medias.length === 0 ? (
              <tr>
                <td colSpan="7" className="text-center text-secondary py-4">No hay medias disponibles</td>
              </tr>
            ) : (
              medias.map((media) => (
                <tr key={media._id} className="custom-table-row">
                  <td className="px-4 py-4 font-monospace text-secondary border-secondary border-opacity-10" style={{ fontSize: '0.75rem' }}>{media.serial}</td>
                  <td className="px-4 py-4 border-secondary border-opacity-10">
                    <div className="rounded overflow-hidden bg-dark shadow-sm position-relative custom-poster-container" style={{ width: '48px', height: '64px' }}>
                      <img alt="Movie Poster" className="w-100 h-100 object-fit-cover custom-poster" src={media.imagenPortada || 'https://via.placeholder.com/48x64'} />
                    </div>
                  </td>
                  <td className="px-4 py-4 border-secondary border-opacity-10" style={{ maxWidth: '320px' }}>
                    <div className="fw-bold text-light mb-1" style={{ fontSize: '1.125rem', letterSpacing: '-0.025em' }}>{media.titulo}</div>
                    <div className="text-secondary fw-light mb-2 text-truncate" style={{ fontSize: '0.75rem' }}>{media.sinopsis}</div>
                    <div className="d-flex gap-2">
                      <span className="badge bg-dark border border-secondary text-secondary" style={{ fontSize: '0.55rem', letterSpacing: '0.1em' }}>{media.tipo?.nombre || 'Tipo'}</span>
                      <span className="text-secondary fw-medium" style={{ fontSize: '0.65rem' }}>{media.anoEstreno}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4 border-secondary border-opacity-10">
                    <span className="fw-semibold text-secondary" style={{ fontSize: '0.75rem' }}>{media.genero?.nombre || 'Género'}</span>
                  </td>
                  <td className="px-4 py-4 border-secondary border-opacity-10">
                    <div className="fw-medium text-light" style={{ fontSize: '0.875rem' }}>{media.director?.nombres || 'Director'}</div>
                    <div className="text-secondary text-uppercase mt-1" style={{ fontSize: '0.65rem', letterSpacing: '0.05em' }}>{media.productora?.nombre || 'Productora'}</div>
                  </td>
                  <td className="px-4 py-4 border-secondary border-opacity-10">
                    <span className="badge rounded-pill border border-success border-opacity-25 text-success" style={{ backgroundColor: 'rgba(25, 135, 84, 0.1)', fontSize: '0.65rem', padding: '0.35rem 0.65rem' }}>ACTIVO</span>
                  </td>
